...
Shearprofile(1, 201:1001) = (abs(SFE(201:1001)) * Q3) ./ (Iprofile(1,500) * tabtotal);
...
Shearcenprofile(201:1001) = (abs(SFE(201:1001)) * Qcen3) ./ (Iprofile(1,600) * 2*1.27);

%thin plate buckling
fish = 0.2;
E = 4000; %N/mm
Diaphragmlos = [1, 100, 200, 280, 375, 475, 601, 726, 826, 921, 1001, 1101, 1201]; %diapharagm locations
...
plot(1:n+1,-top_middle_flange,"cyan-")
plot(sixs, "magenta-")
plot(1:1400,zeros(1,1400), "black-")
xlabel('Length (mm)')
ylabel('Flextural Stress (Mpa)')
title("Flextural Stress")
legend('Bridge Flexture','failure top middle flange', 'failure by compressesion')
hold off
g_fail(1:n+1) = 2;
figure
subplot(1,2,1)
...
subplot(1,2,2)
hold on; grid on; grid minor;
plot(1:n+1,shearbuckling,"black-")
plot(1:n+1,Shearcenprofile, "magenta-")
plot(shrfail, 'red-')
xlabel('Length (mm)')
ylabel('Shear Stress at Centroid(Mpa)')
title("Shear Centroid")
legend('shear at cen', 'failure shear buckling', 'failure shear')
hold off
% 
